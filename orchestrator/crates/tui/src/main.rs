...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let team_lookup: Option<std::sync::Arc<dyn orchestrator_core::team_manager::TeamLookup>> = Some(
    ...
    tokio::spawn(async move {
        if let Err(e) = orchestrator_core::server::run(&server_addr, event_tx, cmd_rx, Some(mcp_for_server), Some(mgr_for_server), Some(cfg_for_server), team_lookup).await {
